...
namespace Torphi.Components.AutoDataLayer.MethodBuilders 
{
	internal class QueryMethodBuilder : MethodBuilderBase 
	{
		...
		public void ResolveFields(string aliasPrefix, Type messageType, Type defaultMessageType, ArrayList listOfUsedMessages, ArrayList listOfFields) {
			foreach(FieldInfo field in messageType.GetFields()) {
				...
				if (! field.IsLiteral && Helpers.CheckISValueType(field.FieldType)) { //ValueType ou Tristate
					...
					if (!field.FieldType.IsArray) {
						...
						object bindable = SuperPag.Framework.Helper.Attributes.GetSingleAttribute ( field, typeof(SuperPag.Framework.Data.Mapping.Bindable ));
