...
namespace SuperPag.Helper
{
    public class ServerHttpHtmlRequisition
    {
        ...
        public bool Send()
        {
            return this.Send(Encoding.ASCII.EncodingName);
        }

        public bool Send(string encoding)
        {
            try
            {
                ...
                {
                    foreach (string key in _parameters.AllKeys)
                        param.AppendFormat("{0}={1}&", HttpUtility.UrlEncode(key, Encoding.GetEncoding("iso-8859-1")), HttpUtility.UrlEncode(_parameters[key], Encoding.GetEncoding("iso-8859-1")));
                }
                if (param.Length != 0)
                ...
                Response = Encoding.GetEncoding(encoding).GetString(responseHTTP, 0, responseHTTP.Length);                
            }
            catch (Exception ex)
            {
                Response = ex.Message;
